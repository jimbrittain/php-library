...
if (!function_exists('produceVCard')) {
    ...
    function produceVCard($args)
    {
        ...
        $arr['email'] = (array_key_exists('email', $arr)) ? resolveEmail($arr['email']) : array();
        $arr['telephone'] = (array_key_exists('telephone', $arr)) ? resolveTelephone($arr['telephone']) : array();
        if (array_key_exists('street-address') && array_key_exists('postal-code') && array_key_exists('country-name')) {
            $arr['adr'] = true;
        } else {
            $arr['adr'] = false;
        }
        ...
        $str = '<div class="vcard">';
            ...
            if ($arr['adr']) {
                $str = $str.'<address class="adr">';
                    ...
                    $str = (isset($arr['region']) && !empty($arr['region'])) ? $str.'<span class="region">'.$arr['region'].'</span> ' : $str;
                    $str = (isset($arr['postal-code']) && !empty($arr['postal-code'])) ? $str.'<span class="postal-code">'.$arr['postal-code'].'</span> ' : $str;
                    $str = (isset($arr['country-name']) && !empty($arr['country-name'])) ? $str.'<span class="country-name">'.$arr['country-name'].'</span> ' : $str;
                    //@codingStandardsIgnoreEnd
               $str = $str.'</address>';
            }
           $str = $str.'<dl>';
